...
const SearchResultSearchbar = ({
  onVendorTypeChange = () => {}, // Default to no-op function
  onTradeOrBusinessChange = () => {},
  onRegionChange = () => {},
}) => {
  ...
  const handleChangeRegionValue = (e) => {
    ...
    // onRegionChange(searchTerm);
    setLocalRegion(searchTerm)
  ...
  const handleSearchClick = (e) => {
    if (
     ...
     localSerachTerm === ""  
      ...
      setSelectedSection('');
      onVendorTypeChange(localVendorType);
      onTradeOrBusinessChange(localSerachTerm);
  ...
  const[localVendorType, setLocalVendoeTye]= useState("");
  const[localRegion, setLocalRegion] = useState("");
  const[localSerachTerm, setLocalSerachTerm] = useState("")

  const handleSelectedVendorType = (value) => {
    // setSelectedVendorType(value);
    // onVendorTypeChange(value);
    setLocalVendoeTye(value);
    setVendorClicked(false);
    setShowLeftDivider(true);
    setShowRightDivider(true);
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <input
            ...
            value={localVendorType}
            onKeyDown={handleKeyDown}
            onChange={(e) => {
              // setSelectedVendorType(e.target.value)
              // onVendorTypeChange(e.target.value);
              setLocalVendoeTye(e.target.value)
            }}
      ...
      >
        ...
        >
          ...
          <input
            type="text"
            className="h-6 w-full text-black text-sm font-medium border-none focus:ring-transparent focus:outline-none bg-transparent px-[12px] placeholder:font-medium lg:placeholder:text-sm md:placeholder:text-xs placeholder-[#787878]"
            ...
            value={localSerachTerm}
            onKeyDown={handleKeyDown}
            onChange={(e) =>
              //  setSelectedTradeOrBusiness(e.target.value)
              // onTradeOrBusinessChange(e.target.value)
              setLocalSerachTerm(e.target.value)
            }
            placeholder="Search by service or material"
          />
          {!localSerachTerm.trim() && error && (
            <Tooltip
              title="This field is required"
              arrow
              placement="top-end"
